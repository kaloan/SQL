...
SELECT * FROM (SELECT DISTINCT MS.NAME, COUNT(*) AS CNT
FROM MOVIESTAR MS
JOIN STARSIN S ON S.STARNAME = MS.NAME
WHERE MS.GENDER = 'F'
GROUP BY MS.NAME) AS STMV
WHERE CNT >= ALL	(SELECT COUNT(*) AS CNT
					FROM MOVIESTAR MS
					JOIN STARSIN S ON S.STARNAME = MS.NAME
					WHERE MS.GENDER = 'F'
					GROUP BY MS.NAME)
...
FROM MOVIESTAR MS
JOIN STARSIN STIN ON STIN.STARNAME = MS.NAME
JOIN MOVIE MV ON MV.TITLE = STIN.MOVIETITLE AND MV.YEAR = STIN.MOVIEYEAR AND MV.INCOLOR = 'Y'
WHERE MS.NAME NOT LIKE '%Jr.%'
ORDER BY MS.BIRTHDATE DESC, MS.NAME ASC
...
SELECT * FROM (SELECT DISTINCT NAME, ADDRESS
FROM STUDIO
...
FROM STUDIO
JOIN MOVIE ON STUDIONAME=NAME
WHERE INCOLOR = 'N') AS REQINFO
ORDER BY ADDRESS

SELECT DISTINCT NAME, ADDRESS
FROM STUDIO
JOIN MOVIE M1 ON M1.STUDIONAME=NAME AND M1.INCOLOR = 'Y'
JOIN MOVIE M2 ON M2.STUDIONAME=NAME AND M2.INCOLOR = 'N'
ORDER BY ADDRESS
